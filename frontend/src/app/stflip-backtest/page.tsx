...
import { stflipBacktestService, } from '@/services/stflip-backtest.service';
...
export default function StflipBacktestPage() {
  ...
  const loadRuns = async () => {
    try { setRuns(await stflipBacktestService.list()); } catch { /* ignore */ }
  };
  ...
  const handleRun = async () => {
    ...
    try {
      const run = await stflipBacktestService.run(fromDate, toDate);
      setStatus('Running… auto-refreshing every 5s');
      const poll = setInterval(async () => {
        try {
          const updated = await stflipBacktestService.get(run._id);
    ...
    <AppShell>
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Strategy A — Relaxed ST Flip</h1>
            <p className="text-sm text-gray-400">
              Supertrend flip trigger · window 9:30–14:00 · VIX≤20 · RSI 52/48 · 2 trades/day
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg p-3 flex flex-wrap gap-3 text-xs">
          <span className="bg-yellow-900 text-yellow-300 px-2 py-0.5 rounded">Supertrend flip trigger</span>
          <span className="bg-blue-900 text-blue-300 px-2 py-0.5 rounded">9:30–14:00</span>
          <span className="bg-purple-900 text-purple-300 px-2 py-0.5 rounded">VIX ≤ 20</span>
          <span className="bg-green-900 text-green-300 px-2 py-0.5 rounded">RSI 52/48</span>
          <span className="bg-orange-900 text-orange-300 px-2 py-0.5 rounded">2 trades/day</span>
          <span className="bg-red-900 text-red-300 px-2 py-0.5 rounded">25-min cooldown</span>
          <span className="bg-indigo-900 text-indigo-300 px-2 py-0.5 rounded">CPR ≤ 1.0%</span>
        </div>
                ...
                <div
                  ...
                  onClick={async () => {
                    if (r.status === 'COMPLETED') {
                      const full = await stflipBacktestService.get(r._id);
                      setSelected(full);
                    }
                  }}
