...
import { hftBacktestService, type HBRun, type HBTrade } from '@/services/hft-backtest.service';
...
export default function HftBacktestPage() {
  const [fromDate, setFromDate] = useState(TWO_YEARS_AGO);
  const [toDate,   setToDate]   = useState(TODAY);
  const [runs,     setRuns]     = useState<HBRun[]>([]);
  const [selected, setSelected] = useState<HBRun | null>(null);
  const [loading,  setLoading]  = useState(false);
  const [status,   setStatus]   = useState('');

  const loadRuns = async () => {
    try { setRuns(await hftBacktestService.list()); } catch { /* ignore */ }
  };
  ...
  const handleRun = async () => {
    ...
    try {
      const run = await hftBacktestService.run(fromDate, toDate);
      setStatus('Running… auto-refreshing every 5s');
      const poll = setInterval(async () => {
        try {
          const updated = await hftBacktestService.get(run._id);
    ...
    <AppShell>
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">HFT Option Backtest</h1>
            <p className="text-sm text-gray-400">
              Strategy D+A — EMA Crossover trigger · 9:30–14:00 window · 2 trades/day · VIX≤20
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg p-3 flex flex-wrap gap-3 text-xs">
          ...
          <span className="bg-purple-900 text-purple-300 px-2 py-0.5 rounded">Entry 9:30–14:00</span>
          <span className="bg-green-900 text-green-300 px-2 py-0.5 rounded">Max 2 trades/day</span>
          <span className="bg-orange-900 text-orange-300 px-2 py-0.5 rounded">VIX ≤ 20</span>
          <span className="bg-red-900 text-red-300 px-2 py-0.5 rounded">25-min cooldown</span>
          <span className="bg-indigo-900 text-indigo-300 px-2 py-0.5 rounded">RSI 52/48</span>
          <span className="bg-gray-700 text-gray-300 px-2 py-0.5 rounded">CPR ≤ 1.0%</span>
        </div>
                ...
                <div
                  ...
                  onClick={async () => {
                    if (r.status === 'COMPLETED') {
                      const full = await hftBacktestService.get(r._id);
                      setSelected(full);
                    }
                  }}
