...
import { vwapScalpService, } from '@/services/vwap-scalp.service';
...
export default function VwapScalpPage() {
  ...
  const loadRuns = async () => {
    try { setRuns(await vwapScalpService.list()); } catch { /* ignore */ }
  };
  ...
  const handleRun = async () => {
    ...
    try {
      const run = await vwapScalpService.run(fromDate, toDate);
      setStatus('Running… auto-refreshing every 5s');
      const poll = setInterval(async () => {
        try {
          const updated = await vwapScalpService.get(run._id);
    ...
    <AppShell>
      <div className="p-6 space-y-6 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Strategy C — VWAP Scalp</h1>
            <p className="text-sm text-gray-400">
              VWAP bounce entry · SL 30% · TP 1.5× · 3 trades/day
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg p-3 flex flex-wrap gap-3 text-xs">
          <span className="bg-yellow-900 text-yellow-300 px-2 py-0.5 rounded">VWAP bounce trigger</span>
          <span className="bg-blue-900 text-blue-300 px-2 py-0.5 rounded">SL 30% (tighter)</span>
          <span className="bg-purple-900 text-purple-300 px-2 py-0.5 rounded">Partial TP 1.5×</span>
          <span className="bg-green-900 text-green-300 px-2 py-0.5 rounded">9:45–14:00</span>
          ...
          <span className="bg-indigo-900 text-indigo-300 px-2 py-0.5 rounded">VIX ≤ 22</span>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg p-4 space-y-4">
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <button
              onClick={handleRun}
              disabled={loading}
              className="flex items-center gap-2 px-4 py-1.5 bg-orange-600 hover:bg-orange-500 disabled:opacity-50 text-white rounded text-sm"
            >
                ...
                <div
                  ...
                  onClick={async () => {
                    if (r.status === 'COMPLETED') {
                      const full = await vwapScalpService.get(r._id);
                      setSelected(full);
                    }
                  }}
                  className={`flex items-center justify-between p-3 rounded cursor-pointer transition-colors ${selected?._id === r._id ? 'bg-orange-900' : 'bg-gray-750 hover:bg-gray-700'}`}
                >
